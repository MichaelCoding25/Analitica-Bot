...
class Errors(commands.Cog):
        ...
        if isinstance(error, ignored):
            ...
            try:
                return await ctx.author.send(
                    f"```css\n [ERROR] {ctx.command} can not be used in Private" f" Messages. ```"
                )
            except:
                pass
            ...
            try:
                return await ctx.author.send(
                    f"```css\n [ERROR] {ctx.command} can not be used" f" in Private Messages. ```"
                )
            except:
                pass
    ...
    async def get_user_stats_handler(self, ctx, error):
        ...
        print(
            f'{error} was raised for {ctx.command}, at {datetime.today().strftime("%b %d %Y %H:%M")}'
        )
